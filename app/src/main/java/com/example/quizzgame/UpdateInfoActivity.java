...
//                databse.updateNameUser(et_name.getText().toString(), idUser);
                if (!et_name.getText().toString().equals("")) {
                    database.updateNameUser(et_name.getText().toString(), idUser);
    ...
    public void dialogConfirmDeleteUser() {
        ...
        dialog.setContentView(R.layout.dialog_close_user);
            ...
            public void onSuccess(DocumentSnapshot documentSnapshot) {
                if (documentSnapshot.exists()) {
                    if (documentSnapshot.getString("password").equals(pass3.getText().toString())) {
                        database.updatePasswordUser(pass1.getText().toString(), idUser);
                        SQLiteDatabase db = openOrCreateDatabase("statelogin", MODE_PRIVATE, null);
                        db.delete("login", null, null);
                        db.close();
                        Intent intent = new Intent(UpdateInfoActivity.this, LoginActivity.class);
